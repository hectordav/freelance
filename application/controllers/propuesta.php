...
class Propuesta extends CI_Controller {
	...
	public function cuerpo_propuesta(){
		if ($this->session->userdata('logueado')){
			...
			$id_proyecto=$data[1];
			$id_status_mensaje=1;
		$datos=$this->propuesta_model->get_propuesta_id($id_usuario_envia,$id_proyecto);
		$datos_propuesta=$this->propuesta_model->get_propuesta_id_para_aceptarla($id_usuario_envia,$id_proyecto);
				foreach ($datos_propuesta->result() as $data) {
					$id_propuesta=$data->id_propuesta;
				}
				$this->propuesta_model->actualizar_propuesta_leido($id_propuesta,$id_status_mensaje);
	...
	public function liberar_dinero(){
		if ($this->session->userdata('logueado')){
			$id_det_mensaje=$this->uri->segment(3);
				$data_usuario = array('id_usuario' =>$this->session->userdata('id'),
					  ...
					  'propuesta'=>$this->propuesta_model->buscar_propuesta_id_usuario($this->session->userdata('id')));
		$id_proyecto=$this->input->post('txt_id_proyecto');
		$usuario_envia=$this->input->post('txt_cliente_envia');
		$usuario_recibe=$this->input->post('txt_cliente_receptor');
		...
		foreach ($data_propuesta->result() as $data) {
			$usuario_envia=$data->id_usuario_recibe;
			$usuario_recibe=$data->id_usuario_envia;
		}
		$data_dep_gar=$this->deposito_garantia_model->get_deposito_garantia($usuario_envia, $usuario_recibe);

		foreach ($data_dep_gar->result() as $data) {
			$id_deposito=$data->id;
		}

		$data_usuario_puntaje=$this->usuario_model->get_usuario_id($usuario_recibe);
		foreach ($data_usuario_puntaje->result() as $data) {
			$puntaje_anterior=$data->usuario_puntaje;
		}
		$puntaje_nuevo=$puntaje+$puntaje_anterior;
		$this->usuario_model->actualizar_usuario_puntaje($usuario_recibe,$puntaje_nuevo);
		$this->propuesta_model->liberar_deposito_garantia_id_usuario_recibe($id_deposito);
		$this->comentario_usuario_model->guardar_comentario($id_proyecto, $usuario_recibe,$comentario, $puntaje);
		#***********************envia el email******************************
		$config = Array(
	'IsSMTP'=>true,
	...
	$data_usuario_2=$this->usuario_model->get_usuario_id($usuario_envia);
	foreach ($data_usuario_2->result() as $data) {
		$login=$data->usuario_login;
	}
	/*$data_id_propuesta=$this->propuesta_model->buscar_propuesta_id($id_proyecto, $usuario_envia, $usuario_recibe);
	foreach ($data_id_propuesta->result() as $data) {
		$id_propuesta=$data->id;
	}*/
	...
	if ($favorito==1) {
		$this->favorito_model->guardar_favorito($usuario_envia,$usuario_recibe);
			redirect('propuesta');
	}
	redirect('propuesta');
		}else{
