<div class="modal fade "id="mnuevomensajepropuesta">
		        <div class="modal-dialog">
		          <div class="modal-content">
					...
					<div class="container">
						<div class="row">
	<form action="<?php echo $this->config->base_url();?>mensaje/nuevo_mensaje_propuesta" method="POST" accept-charset="utf-8">
	<br>
	<?php if ($propuesta): ?>
		<?php foreach ($propuesta->result() as $data): ?>
		<input type="hidden" name="txt_id_proyecto" id="txt_id_proyecto" value="<?=$data->id_proyecto?>">
		<input type="hidden" name="txt_cliente_envia" id="txt_cliente_envia" value=" <?=$data->id_usuario_recibe?>">
		<input type="hidden" name="txt_cliente_receptor" id="txt_cliente_receptor" value="<?=$data->id_usuario_envia?>">
		<?$fecha= date('Y-m-d');?>
